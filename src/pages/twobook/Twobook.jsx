...
const Twobook = () => {
  ...
  const generatePDF = (title, content) => {
    ...
    // Matnni sahifa kengligiga qarab bo'laklarga bo'lish
    const splitContent = doc.splitTextToSize(content, maxWidth);
    
    let cursorY = 30; // Matn boshlanish balandligi
    const lineHeight = 7; // Qatorlar oralig'i

    splitContent.forEach((line) => {
      // Agar qator sahifa oxiriga kelsa, yangi sahifa qo'shish
      if (cursorY + lineHeight > pageHeight - margin) {
        doc.addPage();
        cursorY = margin; // Yangi sahifada tepadan boshlash
      }
    ...
    // 3. Yuklab olish
    doc.save(`${title.substring(0, 20)}.pdf`);
  };
      ...
      <div className="book-card">
        {books.map((item) => (
          <div key={item.id} className="book-question">
            ...
            <button 
              className='twobook-btn' 
              onClick={() => generatePDF(item.title, item.content)}
            >
              PDF yuklab olish
            </button>
          </div>
