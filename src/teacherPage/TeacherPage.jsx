...
const TeacherPage = () => {
  ...
  useEffect(() => {
    // XAVFSIZLIK: Login qilmagan bo'lsa kirgizmaydi
    const auth = sessionStorage.getItem('isTeacherAuth');
    if (auth !== 'true') {
      navigate('/teacher');
      return;
    }
  ...
  return (
    <div data-aos="fade-left" className='teacher-page background' style={{ padding: '20px', minHeight: '100vh' }}>
      ...
      <div className="task-container" style={{ border: '2px solid #3498db', borderRadius: '15px', padding: '20px', marginBottom: '40px', background: '#fff' }}>
        <h2 style={{ marginTop: 0, color: '#3498db' }}>Task 1 Answers</h2>
      ...
      <div className="task-container" style={{ border: '2px solid #2ecc71', borderRadius: '15px', padding: '20px', background: '#fff' }}>
        <h2 style={{ marginTop: 0, color: '#2ecc71' }}>Task 2 Answers</h2>
